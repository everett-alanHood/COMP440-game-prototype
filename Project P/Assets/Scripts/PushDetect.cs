...
public class PushDetect : MonoBehaviour
{
    ...
    public float pushForce = 5f; // Force used to attract the item
    ...
    public void Shoot()
{
    ...
    if (Physics.Raycast(toolRay, out RaycastHit hitInfo, range))
    {
        ...
        if (hitInfo.collider.gameObject.TryGetComponent<Entity>(out Entity portals))
        {
            ...
            isPushing = true; // Start pulling the item
        }
        else
        {
    ...
    void Update()
    {
        ...
        if (isPushing && currentItem != null && item != null)
        {
            ...
            item.AddForce(direction * pushForce, ForceMode.Acceleration);
